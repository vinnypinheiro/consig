...
@Component({
  ...
  styleUrls: ['./contaReceber-view.component.scss'],
...
export class ContaReceberViewComponent extends CommonsForm<ContaReceber>   implements OnInit {
    ...
    contaReceberActive: any;
    totalcontaRecebers: number;
    valortotalcontaRecebers: number;
  ...
  ngOnInit() {
    ...
    getByAssociacao(){
        ...
        this.apiService.getbyassociacao(this.associacaoForm.value).subscribe(response => {
            this.contaReceberActive = response;
            console.log("ContaReceber:   " + this.contaReceberActive);
            this.totalcontaRecebers = this.contaReceberActive.length;
            this.dtInstance.destroy();
            this.dtTrigger.next();
            this.calculaValorTotalContaRecebers(this.contaReceberActive);
    ...
    calculaValorTotalContaRecebers(totalcontaRecebers){

        let valorTotal = [0];
        let valorInicial = 0;
        let valor;

        for (let entry of totalcontaRecebers) {

            valorTotal.push(entry.vlrContaReceber);
        ...
        this.valortotalcontaRecebers = total;
    }
